...
export async function apiCreateGroup(
  ...
  const res = await fetch('/.netlify/functions/group-create', {
    method: 'POST',
...
export async function apiGetGroup(code: string): Promise<StoredGroup> {
  const res = await fetch(
    `/.netlify/functions/group-get?code=${encodeURIComponent(code.toUpperCase().trim())}`
  );
  const data = await res.json();
  if (!res.ok) throw new Error(data.error ?? `HTTP ${res.status}`);
  return data;
...
export async function apiUpdateGroup(
  ...
  const res = await fetch('/.netlify/functions/group-update', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
